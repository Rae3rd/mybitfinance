...
/**
 * Server-side Admin API Service
 * 
 * IMPORTANT: This file should only be imported in server components or API routes.
 * For client components, use the client-side AdminApiClient from @/lib/api/client/adminApi
 */
...
class AdminApiService {
  private async getAuthHeaders(): Promise<HeadersInit> {
    try {
      ...
      // Safely extract role from metadata with proper type checking
      let role = 'user';
  ...
  async addSupportTicketReply(
    id: string,
    message: string,
    isInternal: boolean = false
  ): Promise<any> {
    const response = await this.request(`/admin/support/tickets/${id}/replies`, {
      method: 'POST',
